-- 更新说明
-- 1、tb_loan_apply_attach表中的外键lf_id  修改为tb_loan_apply申请单主表的主键la_id
-- 2、根据申请单预览主表tb_loan_forms 新建申请单预览副表 tb_loan_forms_attach
...
-- 导出 tbp1 的数据库结构
CREATE DATABASE IF NOT EXISTS `tbp1` /*!40100 DEFAULT CHARACTER SET utf8 */;
USE `tbp1`;
...
CREATE TABLE IF NOT EXISTS `tb_loan_apply_attach` (
  `la_id` int(11) unsigned NOT NULL COMMENT '申请主表id',
...
-- 导出  表 tbp1.tb_loan_forms_attach 结构
CREATE TABLE IF NOT EXISTS `tb_loan_forms_attach` (
  `lfa_id` int(11) unsigned NOT NULL AUTO_INCREMENT,
  ...
  `ht_type` varchar(50) NOT NULL COMMENT '填写方式',
  `pdi_values` varchar(1000) DEFAULT NULL COMMENT '数据项值',
  `creatorid` int(11) unsigned NOT NULL COMMENT '创建人id',
  `createtime` datetime NOT NULL COMMENT '创建时间',
  `updatorid` int(11) NOT NULL COMMENT '最后修改人id',
  `updatetime` datetime NOT NULL COMMENT '最后修改时间',
  `enable` varchar(1) NOT NULL DEFAULT 'Y' COMMENT '是否可见',
  PRIMARY KEY (`lfa_id`)
...
CREATE TABLE IF NOT EXISTS `tb_role` (
  ...
  `r_status` int(11) unsigned DEFAULT '0' COMMENT '角色状态,0：正常；1：删除',
