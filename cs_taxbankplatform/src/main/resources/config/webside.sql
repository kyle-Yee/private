...
-- ----------------------------
INSERT INTO `tb_resource` VALUES ('1', null, '首页', 'desktop', '0', '/welcome.jsp', '1', 'fa fa-tachometer', '0', '首页', '2016-01-12 17:08:55', '2016-02-25 14:07:48');
...
-- Records of tb_role
-- ----------------------------
INSERT INTO `tb_role` VALUES ('1', '超级管理员', 'administrator', 'Y', '超级管理员', '1',  '2016-01-05 12:07:42', '2016-02-27 22:30:15');
INSERT INTO `tb_role` VALUES ('2', '管理员', 'admin', 'Y', '管理员', '0',  '2016-01-05 12:07:42', '2016-02-27 22:30:22');
INSERT INTO `tb_role` VALUES ('3', '普通用户', 'customer', 'Y', '普通用户', '1',  '2016-02-28 17:09:40', '2016-03-08 22:55:36');
INSERT INTO `tb_role` VALUES ('4', '深圳纳税人', 'nsr4sz', 'Y', '深圳纳税人', '1',  '2016-02-28 17:09:40', '2016-03-08 22:55:36');
INSERT INTO `tb_role` VALUES ('5', '广州纳税人', 'nsr4gz', 'Y', '广州纳税人', '1',  '2016-02-28 17:09:40', '2016-03-08 22:55:36');
...
-- ----------------------------
INSERT INTO `tb_user_info` VALUES ('1', '1', '2016-02-10', '13000000000', 'zhangmengzhi2005@126.com', '广东省广州市中山二路', '2016-02-18 16:43:28');
...
-- 2016-9-21 liwangxiong  start --
-- 修改组织表
-- ----------------------------
-- Table structure for `tb_organizations`
-- ----------------------------
ALTER TABLE `tb_organizations` ADD COLUMN `org_logo_url` VARCHAR(100) COMMENT '组织logo地址' AFTER `org_code`;

-- 2016-9-21 liwangxiong  end --


-- 2016-11-30 xiecui  start --
-- 新建区域级别表
CREATE TABLE `tb_region_class` (
	`rc_id` INT(11) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '区域级别id',
	`rc_name` VARCHAR(50) NOT NULL COMMENT '区域级别名称',
	`rc_description` VARCHAR(200) NULL DEFAULT NULL COMMENT '区域级别描述',
	PRIMARY KEY (`rc_id`)
)
COMMENT='区域级别表'
COLLATE='utf8_general_ci'
ENGINE=InnoDB;
-- 2016-11-30 xiecui  end 


-- 2016-11-29 liwangxiong  start --
--修改组织表
ALTER TABLE `tb_organizations` ADD COLUMN `up_org_id` INT(11) UNSIGNED NULL COMMENT '上级组织id' AFTER `region_id`,
	ADD COLUMN `region_class` INT(11) UNSIGNED NOT NULL COMMENT '区域所属级别' AFTER `ot_id`;
-- 2016-11-29 liwangxiong  end --	

-- 2016-11-30 liwangxiong  start --
--修改角色表
ALTER TABLE `tb_role` ALTER `r_status` DROP DEFAULT;
ALTER TABLE `tb_role` CHANGE COLUMN `r_status` `r_status` VARCHAR(1) NOT NULL COMMENT '角色状态,Y：正常；N：删除' AFTER `r_key`;
-- 2016-11-30 liwangxiong  end 

-- 2016-12-01 xiecui  start --
--修改区域表（添加currentlocation字段）
ALTER TABLE `tb_regions`
	ADD COLUMN `currentlocation` VARCHAR(1) NOT NULL COMMENT '开通状态(Y:默认首页，N:否)' AFTER `enabled`;
-- 2016-11-30 xiecui  end 

-- 2016-12-13 liwangxiong  start --
ALTER TABLE `tb_financial_product`
	ALTER `la_id` DROP DEFAULT,
	ALTER `ld_id` DROP DEFAULT,
	ALTER `lr_id` DROP DEFAULT;	
ALTER TABLE `tb_financial_product`
	CHANGE COLUMN `la_id` `la_id` INT(11) UNSIGNED NOT NULL COMMENT '最高授信额度' AFTER `fp_remove_time`,
	CHANGE COLUMN `ld_id` `ld_id` INT(11) UNSIGNED NOT NULL COMMENT '最长授信期限' AFTER `la_id`,
	CHANGE COLUMN `lr_id` `lr_id` INT(11) UNSIGNED NOT NULL COMMENT '授信利率' AFTER `ld_id`,
	ADD COLUMN `fp_many_credit` VARCHAR(1) NOT NULL COMMENT '多重授信' AFTER `rw_ids`;

ALTER TABLE `tb_loan_amount`
	ALTER `la_name` DROP DEFAULT,
	ALTER `la_code` DROP DEFAULT;
ALTER TABLE `tb_loan_amount`
	COMMENT='授信额度',
	CHANGE COLUMN `la_name` `la_name` VARCHAR(50) NOT NULL COMMENT '授信额度名称' AFTER `la_id`,
	CHANGE COLUMN `la_code` `la_code` VARCHAR(50) NOT NULL COMMENT '授信额度编码' AFTER `la_name`;

ALTER TABLE `tb_loan_deadline`
	ALTER `ld_name` DROP DEFAULT,
	ALTER `ld_code` DROP DEFAULT;
ALTER TABLE `tb_loan_deadline`
	COMMENT='授信期限',
	CHANGE COLUMN `ld_name` `ld_name` VARCHAR(50) NOT NULL COMMENT '授信期限名称' AFTER `ld_id`,
	CHANGE COLUMN `ld_code` `ld_code` VARCHAR(50) NOT NULL COMMENT '授信期限类型编码' AFTER `ld_name`;

ALTER TABLE `tb_loan_rates`
	ALTER `lr_name` DROP DEFAULT,
	ALTER `lr_code` DROP DEFAULT;
ALTER TABLE `tb_loan_rates`
	COMMENT='授信利率',
	CHANGE COLUMN `lr_name` `lr_name` VARCHAR(50) NOT NULL COMMENT '授信利率名称' AFTER `lr_id`,
	CHANGE COLUMN `lr_code` `lr_code` VARCHAR(50) NOT NULL COMMENT '授信利率类型编码' AFTER `lr_name`;

UPDATE `tbp1`.`tb_resource` SET `s_name`='授信额度维护' WHERE  `s_id`=34;
UPDATE `tbp1`.`tb_resource` SET `s_name`='授信利率维护' WHERE  `s_id`=35;
UPDATE `tbp1`.`tb_resource` SET `s_name`='授信期限维护' WHERE  `s_id`=36;

UPDATE `tbp1`.`tb_product_status` SET `ps_name`='上架' WHERE  `ps_id`=2;
UPDATE `tbp1`.`tb_product_status` SET `ps_name`='下架' WHERE  `ps_id`=3;

ALTER TABLE `tb_loan_forms`
	CHANGE COLUMN `la_id` `la_id` INT(11) UNSIGNED NULL DEFAULT NULL COMMENT '授信协议ID（涉税数据）' AFTER `region_id`,
	ADD COLUMN `la_idxy` INT(11) UNSIGNED NULL DEFAULT NULL COMMENT '授信协议ID（信用数据）' AFTER `la_id`;

ALTER TABLE `tb_regions`
	ADD COLUMN `region_code` VARCHAR(50) NOT NULL COMMENT '开通区域code' AFTER `region_name`;

ALTER TABLE `tb_loan_forms`
	CHANGE COLUMN `la_id` `la_id` INT(11) UNSIGNED NULL DEFAULT NULL COMMENT '授权协议ID' AFTER `region_id`,
	DROP COLUMN `la_idxy`;


-- 2016-12-13 liwangxiong  end --	
	
-- 2017-1-6 yaofang start--	

--新增 tb_loan_approve_finalstatus(贷后已完成状态表)
CREATE TABLE IF NOT EXISTS `tb_loan_approve_finalstatus` (
  `lafs_id` int(11) NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `lafs_name` varchar(50) DEFAULT NULL COMMENT '名称',
  `lafs_code` varchar(50) DEFAULT NULL COMMENT '代码',
  `enabled` varchar(1) DEFAULT NULL COMMENT '是否有效',
  `lafs_remark` varchar(100) DEFAULT NULL COMMENT '备注',
  PRIMARY KEY (`lafs_id`)
) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=utf8 COMMENT='贷后已完成状态';

-- 正在导出表  tbp2.tb_loan_approve_finalstatus 的数据：~2 rows (大约)
/*!40000 ALTER TABLE `tb_loan_approve_finalstatus` DISABLE KEYS */;
INSERT INTO `tb_loan_approve_finalstatus` (`lafs_id`, `lafs_name`, `lafs_code`, `enabled`, `lafs_remark`) VALUES
	(1, '授信终止', 'LAFS_SXZZ', 'Y', '银行手动终止'),
	(2, '自动终止', 'LAFS_ZDZZ', 'Y', '到期终止');

--新增 tb_loan_apply_final tb_loan_approve_code 字段
ALTER  table  tb_loan_apply_final  add  `lafs_id` int(11) unsigned DEFAULT NULL COMMENT '贷后完成状态代码';
 
ALTER  table  tb_loan_approve_code  add  `lafs_id` int(11) unsigned DEFAULT NULL COMMENT '贷后完成状态代码';

--新增tb_bankloan_tyle  (贷后评价状态表）
CREATE TABLE IF NOT EXISTS `tb_bankloan_tyle` (
  `blt_id` int(11) NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `blt_name` varchar(50) DEFAULT NULL COMMENT '名称',
  `enabled` varchar(1) DEFAULT NULL COMMENT '是否有效',
  `blt_remark` varchar(100) DEFAULT NULL COMMENT '备注',
  PRIMARY KEY (`blt_id`)
) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=utf8 COMMENT='贷后评价状态表';

--新增 tb_loan_applyEnd(信贷终止表)
CREATE TABLE IF NOT EXISTS `tb_loan_applyend` (
  `lae_id` int(11) unsigned NOT NULL AUTO_INCREMENT COMMENT '记录id',
  `lafs_id` int(11) unsigned DEFAULT NULL COMMENT '贷后完成状态代码',
  `laf_id` int(11) unsigned NOT NULL COMMENT '授信表ID',
  `nsrmc` varchar(100) NOT NULL COMMENT ' 企业名称',
  `nsrbh` varchar(100) NOT NULL COMMENT '纳税人识别号/社会信用统一代码',
  `lae_endDate` date NOT NULL COMMENT '授信终止时间',
  `lae_credit_quota` varchar(100) NOT NULL COMMENT '授信完成额度',
  `lae_overdue_count` int(11) unsigned DEFAULT NULL COMMENT '逾期次数',
  `bankloan_type` int(11) unsigned DEFAULT NULL COMMENT '评价授权',
  ...
  PRIMARY KEY (`lae_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='信贷终止表';


-- 2017-1-6 yaofang end--	
