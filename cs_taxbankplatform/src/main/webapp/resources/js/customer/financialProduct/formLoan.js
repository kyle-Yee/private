...
$(function() {
	...
	$("#btn2").click(function(){
		webside.common.loadPage('/financialProduct/addUI.html?ptdm=2');
	});
	if($("#stopCount").val() == "1" ){
		stopCount();
	}
...
/*	 var gsIdval = $("#financialProduct input[name='gsId']:checked").val();
	 ...
	 if(checkfpName ==true  && checkfpOverlayPcId ==true &&  editor ==true){
		return true;
	}
	return false;
}
 ...
 function submitAll(){//添加界面的提交按钮事件
	 ...
	 if(undefined == sxxyss || "undefined" == sxxyss || null == sxxyss || "" == sxxyss){
		 $("#loanForms #sqxyss_label").attr("productditvalueslist",0); //授权协议ID
		 //1.产品保存、申请单基表保存
		 var submitBoolean = false;
		 var newlfId =0;
		 var ajax_option={
				 async:false,
				 url: sys.rootPath + "/financialProduct/add.html?ptdm=2&sxxyss=0",
				 dataType : "json",
				 success : function(resultdata) {
		 ...
		 var ajax_option={
				 async:false,
				 url: sys.rootPath + "/financialProduct/add.html?ptdm=2&sxxyss="+sxxyss,
				 dataType : "json",
				 success : function(resultdata) {
					 submitBoolean = resultdata.success;
					 newlfId = resultdata.newlfId;
...
//					alert("oldSize"+resultdata.oldSize);
 ...
 function submitUpdateAll(fpId){ //修改界面的提交按钮事件
	 ...
	 if(undefined == sxxyss || "undefined" == sxxyss || null == sxxyss || "" == sxxyss){
		 ...
		 var ajax_option={
			async:false,
			url: sys.rootPath + '/financialProduct/edit.html?ptdm=2&sxxyss='+sxxyss,
			dataType : "json",
			success : function(resultdata) {
				submitBoolean = resultdata.success;
 ...
 function showHtml(){ //预览贷款申请单的生成效果
	 var sxxyss =$('#lf_sqxyss option:selected') .val(); //授权协议ID
	 if (undefined == sxxyss || "undefined" == sxxyss || null == sxxyss || "" == sxxyss) {
		 $("#lf_sqxyss").attr("selected","selected");
	 }else{
